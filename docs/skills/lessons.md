...
In the `type: "ui"` IComponent path, `logos.callModule` returned the C++ QString directly (single-wrapped JSON). After switching to `type: "ui_qml"` (commit 48c1900), the LogosQmlBridge wraps the return value in an additional JSON string layer. `JSON.parse(result)` returns a string, not an object — `obj.success` is `undefined`, array iteration over a string produces phantom entries. **Rule**: always use `callModuleParse()` (the double-unwrap helper) for all `logos.callModule` results in notes_ui QML. Never use `JSON.parse()` directly on callModule results. Exception: plain-string returns like `isInitialized`, `getKeySource`, `getAccountFingerprint` — these need no parse. For `loadNote` which returns plain text or an error JSON, unwrap one layer with `JSON.parse` then check `charAt(0) === '{'` for error detection. (Issues #92, #93, #94 — fixed 2026-04-16.)
